...
const app = new Vue({
  ...
  mounted() {
    ...
    const vue = this;
    const frame = new Frame("fit", 635, 850, '#00000000', '#00000000', srcImg,);
      frame.on("ready", () => {// ES6 Arrow Function - similar to function(){} {ready, assets:"SpecialElite-Regular.ttf", path:"/assets/font"}
        ...
        frame.on("complete", ()=>{
          ...
          const hintCheck = new CheckBox(26, "Aggiungi contorno").alp(.8).pos(0, 70, CENTER, BOTTOM).wire({ target: hint, prop: "visible", input: "checked" });
          const picCheck = new CheckBox(26, "Aggiungi figura").alp(.8).pos(0, 30, CENTER, BOTTOM).wire({ target: pic, prop: "visible", input: "checked" });
